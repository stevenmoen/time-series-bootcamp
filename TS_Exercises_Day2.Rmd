...
```{r}
# Check ADF test for non-stationarity
adf.test(globtemp)
```

Data looks non-stationary. Let's convert to an approximate percentage change.

```{r}
gt_diff = diff(globtemp)
```
...
MA lags could be 1, 2, 4.

```{r}
pacf(gt_diff)
```

AR lags could be 1,2,3. Let's test all of these.

```{r}
# MA = 1
# arima(globtemp, order = c(1,1,1))
m11 = arima(gt_diff, order = c(1,0,1))
m21 = arima(gt_diff, order = c(2,0,1))
m31 = arima(gt_diff, order = c(3,0,1))

# MA = 2
m12 = arima(gt_diff, order = c(1,0,2))
m22 = arima(gt_diff, order = c(2,0,2))
m32 = arima(gt_diff, order = c(3,0,2))

# MA = 4
m14 = arima(gt_diff, order = c(1,0,4))
m24 = arima(gt_diff, order = c(2,0,4))
m34 = arima(gt_diff, order = c(3,0,4))
...
Need to compare an ARMA(1,1) with an ARMA(1,4)
